import type { MaybeString } from '../string';

/**
 * Mask a personal name with `*`.
 *
 * Length 2 keeps the last character (`张三` → `*三`). Longer names keep the
 * first and last characters and mask the middle. Nullish input yields `''`.
 *
 ...
 */
export const starlizeName = (name: MaybeString): string => {
 ...
 */
export const starlizeCard = (card: MaybeString, front: number, after: number): string => {
